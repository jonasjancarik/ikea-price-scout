document.addEventListener('DOMContentLoaded', function () {
    ...
    function getSelectedCountries() {
        ...
        // Add home country first
        if (selectedHomeCountry) {
            const site = ikeaSites[selectedHomeCountry];
            if (site) {
                selected.push({
                    country: selectedHomeCountry,
        ...
        chrome.storage.sync.get(['selectedCountries'], function(result) {
            if (result.selectedCountries) {
                result.selectedCountries.forEach(saved => {
                    if (saved.isHome) {
                        // Set home country
                        const homeRadio = document.querySelector(`input[name="homeCountry"][value="${saved.country}"]`);
                        if (homeRadio) {
                            homeRadio.checked = true;
                            selectedHomeCountry = saved.country;
                            nextButton.disabled = false;
                            nextButton.style.backgroundColor = '#0051ba';
                        }
                    } else {
                        // Set comparison country
                        const checkbox = document.querySelector(`input[data-country="${saved.country}"]`);
                        if (checkbox) {
                            checkbox.checked = true;
                            selectedComparisonCountries.add(saved.country);
                            const select = checkbox.closest('.country-option').querySelector('.language-select');
                            if (select) {
    ...
    function updateStatus() {
        ...
        const comparisonCount = selected.filter(s => !s.isHome).length;
        
        if (comparisonCount === 0) {
            statusMessage.textContent = 'Není vybrána žádná země pro porovnání. Rozšíření nebude nic dělat.';
            statusMessage.style.color = 'red';
        } else {
            statusMessage.textContent = `${comparisonCount} ${comparisonCount === 1 ? 'země vybrána' : 'země vybrány'} pro porovnání.`;
